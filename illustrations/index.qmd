...
**[DEGRONOPEDIA](https://degronopedia.com/){target="_blank"}**, a web server for
degrons, the short sequence motifs that mark a protein for destruction by the
ubiquitin–proteasome system. It is built by the Pokrzywa Lab (Laboratory of
Protein Metabolism) at IIMCB and searches degrons across eleven model proteomes.

::: {.tag-row}
[Logo design]{.pill} [Web server]{.pill} [Proteostasis]{.pill}
:::
...
**To show the logo rather than describe it,** add the file to
`assets/img/illustrations/` and give this section a gallery block like the ones
above. Check first that the lab is happy for it to appear here.
:::
